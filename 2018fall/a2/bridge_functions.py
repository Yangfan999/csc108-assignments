...
def format_data(data: List[List[str]]) -> None:
    ...
    for i in data:
        span = []
        for b in range(len(i[9])):
            if i[9][b] == ';':
                c = b - 1
                while i[9][c] != '=':
                    c -= 1
                span.append(float(i[9][c+1:b]))
        bci = []
        for k in i[13:]:
            if k != '':
                bci.append(float(k))
        temp = [ids, i[1], i[2], float(i[3]), float(i[4]), i[5],
                i[6], i[7], int(i[8]), span, float(i[10]), i[11], bci]
        ids += 1
        res.append(temp)
    data.clear()
    for z in res:
...
def get_average_bci(bridge_data: List[list], bridge_id: int) -> float:
    ...
    for i in bridge_data:
        if i[ID_INDEX] == bridge_id:
            return sum(i[BCIS_INDEX])/len(i[BCIS_INDEX])
    return 0.0
...
def get_distance_between(bridge1: list, bridge2: list) -> float:
    ...
    1.968
    """
    return calculate_distance(bridge1[LAT_INDEX],
                              bridge1[LON_INDEX], bridge2[LAT_INDEX], bridge2[LON_INDEX])
...
def find_closest_bridge(bridge_data: List[list], bridge_id: int) -> int:
    ...
    temp = bridge_data[0] if bridge_data[0] != bridge else bridge_data[1]
    dis = get_distance_between(temp, bridge)
    for i in bridge_data:
        if get_distance_between(i, bridge) < dis and i != bridge:
            temp = i
            dis = get_distance_between(i, bridge)
    return temp[ID_INDEX]
...
def find_bridges_in_radius(bridge_data: List[list], lat: float, long: float,
    ...
    res = []
    for i in bridge_data:
        if calculate_distance(i[LAT_INDEX], i[LON_INDEX], lat, long) <= distance:
            res.append(i[ID_INDEX])
    return res
...
def get_bridges_with_bci_below(bridge_data: List[list], bridge_ids: List[int],
    ...
    temp = []
    for i in bridge_ids:
        bci = get_bridge(bridge_data, i)
        if bci[BCIS_INDEX] and bci[BCIS_INDEX][0] <= bci_limit:
            temp.append(bci[ID_INDEX])
    return temp
...
def get_bridges_containing(bridge_data: List[list], search: str) -> List[int]:
    ...
    res = []
    for i in bridge_data:
        if search.upper() in i[NAME_INDEX].upper():
            res.append(i[ID_INDEX])
    return res
...
def assign_inspectors(bridge_data: List[list], inspectors: List[List[float]],
    ...
    for i in inspectors:
        ...
        for k in temp:
            acc += k
        hb = find_bridges_in_radius(bridge_data, i[0], i[1], HIGH_PRIORITY_RADIUS)
        mb = find_bridges_in_radius(bridge_data, i[0], i[1], MEDIUM_PRIORITY_RADIUS)
        lb = find_bridges_in_radius(bridge_data, i[0], i[1], LOW_PRIORITY_RADIUS)
        rhb = get_bridges_with_bci_below(bridge_data, hb, HIGH_PRIORITY_BCI)\
            + get_bridges_with_bci_below(bridge_data, mb, MEDIUM_PRIORITY_BCI)\
            + get_bridges_with_bci_below(bridge_data, lb, LOW_PRIORITY_BCI)
        sesp = []
        inx = 0
        while len(sesp) < max_bridges and inx < len(rhb):
            if rhb[inx] not in acc:
                sesp.append(rhb[inx])
            inx += 1
        temp.append(sesp)
    return temp
